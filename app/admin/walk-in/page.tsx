...
export default function WalkInPage() {
  ...
  const { state, mode, addWalkIn } = useOperations();
  const availableRooms = state.rooms.filter((room) => isRoomOperationallyAvailable(room.status) && (mode === "demo" || room.inventoryValid));
  const [error, setError] = useState("");
  const [form, setForm] = useState({ firstName: "", lastName: "", phone: "", document: "", guestCount: 1, roomId: availableRooms[0]?.id ?? "", checkIn: hostelDate(), checkOut: hostelDate(1), nightlyRate: mode === "demo" ? DEFAULT_REFERENCE_RATE_ARS : availableRooms[0]?.baseRate ?? 0, amountPaid: 0, paymentMethod: "cash" as PaymentMethod, notes: "" });
    ...
    <>
      ...
      <div className="admin-form-layout">
        <form className="admin-form-card" onSubmit={submit}>
          ...
          <div className="admin-field-grid">
            <label>Habitación<select required value={form.roomId} onChange={(e) => { const room = availableRooms.find((item) => item.id === e.target.value); setForm({ ...form, roomId: e.target.value, nightlyRate: room?.baseRate ?? form.nightlyRate }); }}><option value="">Seleccionar</option>{availableRooms.map((room) => <option value={room.id} key={room.id}>{room.displayName} · hasta {room.capacity}</option>)}</select></label>
          ...
          <div className="admin-field-grid">
            <label>Tarifa acordada por noche<input required min="1" step="100" type="number" value={form.nightlyRate} onChange={(e) => setForm({ ...form, nightlyRate: Number(e.target.value) })} /></label>
          ...
          {!state.rooms.length ? <p className="admin-form-error" role="alert">Todavía no hay habitaciones configuradas. Completá el inventario desde Configuración.</p> : !availableRooms.length ? <p className="admin-form-error" role="alert">No hay una habitación activa con tipo, tarifa, capacidad y estado disponible.</p> : null}
